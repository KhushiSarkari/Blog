<template>
    <div id="add-blog">
        
        <form v-if="!submitted">
            <h2>Add a New Blog Post</h2>
            <label>Title:</label>
            <input type="text" v-model.lazy="blog.title" required />
            <label>Content:</label>
            <textarea v-model.lazy.trim="blog.content"></textarea>
            <label>Author:</label>
            <select v-model="blog.author">
                <option v-for="author in authors">{{ author }}</option>
            </select>
            <div id="checkboxes">
                <p>Categories:</p>
                <label>Political</label>
                <input type="checkbox" value="political" v-model="blog.categories" />
                <label>Science</label>
                <input type="checkbox" value="science" v-model="blog.categories" />
                <label>History</label>
                <input type="checkbox" value="history" v-model="blog.categories" />
                <label>Travel</label>
                <input type="checkbox" value="travel" v-model="blog.categories" />
                <label>Entertainment</label>
                <input type="checkbox" value="entertainment" v-model="blog.categories" />
            </div>
            
            <hr />
            <br>
            <button v-on:click.prevent="post">Add Blog</button>
        </form>
        <div v-if="submitted">
            <h3>Post added</h3>
        </div>
    </div>
</template>

<script>
// Imports

export default {
    data () {
        return {
            blog: {
                title: '',
                content: '',
                categories: [],
                author: ''
            },
            authors: ['Eclectic KS', 'My Code Stinks', 'Vini B'],
            submitted: false
        }
    },
    methods: {
        post: function(){
            this.$http.post('https://blog-faf3e.firebaseio.com/posts.json', this.blog).then(function(data){
                console.log(data);
                this.submitted = true;
            });
        }
    }
}

</script>

<style scoped>
#add-blog *{
    box-sizing: border-box;
}
#add-blog{
    margin: 20px auto;
    max-width: 600px;
    padding: 20px;
}
label{
    display: block;
    margin: 20px 0 10px;
}
input[type="text"], textarea, select{
    display: block;
    width: 100%;
    padding: 8px;
}
textarea{
    height:200px;
}
#preview{
    padding: 10px 20px;
    border: 1px dotted #ccc;
    margin: 30px 0;
}
h3{
    margin-top: 10px;
}
#checkboxes input{
    display: inline-block;
    margin-right: 10px;
}
#checkboxes label{
    display: inline-block;
    margin-top: 0;
}
hr{
    display: none;
}
button{
    display: block;
    margin: 20px 0;
    background: #002200;
    color: #fff;
    border: 0;
    padding: 14px;
    border-radius: 4px;
    font-size: 18px;
    cursor: pointer;
}
</style>
